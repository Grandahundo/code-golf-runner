...
Write `solve349.py` that is **SHORTER** than `baseline.py` while passing all tests.

## How to Work

1. Read `baseline.py` — understand what `p(grid)` does
2. Write `solve349.py` with same `p(grid)` → 2D list signature
3. Run `python verify.py solve349.py` to test
