...
Write `solve265.py` that is **SHORTER** than `baseline.py` while passing all tests.

## How to Work

1. Read `baseline.py` — understand what `p(grid)` does
2. Write `solve265.py` with same `p(grid)` → 2D list signature
3. Run `python verify.py solve265.py` to test
